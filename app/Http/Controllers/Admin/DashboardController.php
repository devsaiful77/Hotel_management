...
class DashboardController extends Controller
{
    public function index(){
        ...
        if(Auth::user()->role_id == 3) {
            ...
            $totalDeliveredOrder = Order::where('rstrt_slug', $restaurant->slug)->where('payment_status', 1)->count();
            $totalDeliveryTime = Order::where('rstrt_slug', $restaurant->slug)->where('payment_status', 1)->sum('avg_time');
            $time = $totalDeliveryTime / $totalDeliveredOrder;

            $avgTime = (new OrderController)->getAVGTimeDiff($time);
            // return $avgTime = Order::where('rstrt_slug', $restaurant->slug)->where('payment_status', 1)
            //                 ->selectRaw('AVG(TIME_TO_SEC(created_at) - TIME_TO_SEC(updated_at))')
            //                 ->get();
            ...
            return view('admin.dashboard.home', compact('avgTime','no_of_emp', 'no_of_delivered_orders', 'no_of_cancel_order','income', 'vat', 'expense', 'monthly_incomes', 'monthly_expenses', 'monthly_profits'));
        }
