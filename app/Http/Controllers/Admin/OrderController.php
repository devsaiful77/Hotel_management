...
class OrderController extends Controller
{
    ...
    {
        // $now_timestamp = date('Y-m-d H:i:s',strtotime(Carbon::now()));
        $now_timestamp = strtotime(date('Y-m-d H:i:s'));
        return $diff_timestamp = $now_timestamp - strtotime($date);
    }

    public function getAVGTimeDiff($time)
    {
        // return $time;
        if ($time < 60) {
            return 'few seconds';
        } elseif ($time > 60 && $time < 3600) {
            return round($time / 60) . ' mins';
        } elseif ($time > 3600 && $time < 86400) {
            return round($time / 3600) . ' hours';
        } elseif ($time > 86400 && $time < (86400 * 30)) {
            return round($time / (86400)) . ' days';
        } elseif ($time > (86400 * 30) && $time < (86400 * 365)) {
            return round($time / (86400 * 30)) . ' months';
        } else {
            return round($time / (86400 * 365)) . ' years';
        }
    }
        ...
        if ($action == 3 || $action == 5) {
            ...
            foreach ($employees as $employee) {
                ...
                if ($action == 3 && $employee->userInfo->role_id == 4) {
                    ...
                    $employee->userInfo->notify(new ServerRequest($order));
                }
                // Read the notification
                $unreads = $employee->userInfo->unreadNotifications;
                foreach ($unreads as $unread) {
    ...
    public function typrWiseOrdeOrderServe(Request $request)
    {
        ...
        if (Auth::user()->role_id == 3 || Auth::user()->role_id == 4) {
            // dd($request->CustType);  daynin ba takwaey
            $slug = Employee::where('user_id', Auth::user()->id)->first()->rstrt_slug;
    ...
    public function serveData($rstrtSlug, $orderSlug, $action)
    {
        ...
        if ($action == 4 || $action == 5) {
            ...
            $time = $this->getDateTimeDiff($order->created_at);
    ...
    public function delivered()
    {
        ...
        if (Auth::user()->role_id == 3) {
            ...
            // $avg_time = Order::where('slug', $orderSlug)->first()->avg_time;
            // $avgTime = $this->getAVGTimeDiff($avg_time);

            $orders = Order::where('rstrt_slug', $slug)->where('status', 4)->where('type', 1)->where('created_at', '>=', $from)->orderBy('id', 'DESC')->get();

            // foreach($orders as $avgTime){
            // $time = $this->getAVGTimeDiff($avgTime->avg_time);
            // $avgTime = $time;
            //  echo $time;
            //  echo $avgTime->avg_time;
            // }
            return view('admin.order.delivery.all', compact('orders', 'slug', 'from', 'to'));
        }
    }

    public function typrWiseOrderDelivered(Request $request)
    {
        if (Auth::user()->role_id == 2) {
            $client = Client::where('email', Auth::user()->email)->first();
            $restaurant = $client->restaurantInfo;
        ...
        if (Auth::user()->role_id == 3) {
            ...
            $orders = Order::where('rstrt_slug', $slug)->where('status', 4)->where('type', $request->CustType)->where('created_at', '>=', $from)->orderBy('id', 'DESC')->get();

            if ($request->CustType == 1) {
                return view('admin.order.delivery.all', compact('orders', 'slug', 'from', 'to'));
            }
            return view('admin.order.delivery.type2', compact('orders', 'slug', 'from', 'to'));
        }
    }
    ...
    public function addToCartMenu($id, $rstrt_slug)
    {
        ...
        $addonsProduct = 0;
        if ($menu->addons == true) {
            $addonsProduct = Menu::where('cate_id', 25)->where('rstrt_slug', $rstrt_slug)->get();
        }

        $sizes = $menu->size;
        $prices = $menu->price;
        $actualCost = $menu->actual_cost;
        $allSize = 0;

        if ($menu->size != 0) {
            $actCost = explode(',', $actualCost);
        ...
        return response()->json(['menu' => $menu, 'actCost' =>$actCost, 'allSize' => $allSize, 'allPrice' => $allPrice, 'addonsProduct' => $addonsProduct, 'combainArray' => $combainArray]);
    }
